...
// ── 狀態追蹤（process-level，適用於快速連續訊息）──
const feedbackMode = new Map(); // userId -> timestamp
const FEEDBACK_TTL = 5 * 60 * 1000; // 5 分鐘

const FEEDBACK_THANKS = "謝謝你的寶貴回饋！我們會認真參考 🌿\n後續若有新版本更新，我們會第一時間通知你！";
...
export default async function handler(req, res) {
  ...
  for (const event of req.body.events || []) {
    ...
    const inFeedback = feedbackMode.has(uid) && (Date.now() - feedbackMode.get(uid)) < FEEDBACK_TTL;

    try {
      // ── 圖片訊息 ──────────────────────────────────────
      if (msgType === "image") {
        if (inFeedback) {
          feedbackMode.delete(uid);
          logToSheet(uid, `[內測截圖 imageId:${event.message.id}]`, "(內測回饋截圖)");
          await sendReply(event.replyToken, FEEDBACK_THANKS);
        }
        // 非回饋模式的圖片，忽略
        continue;
      }
      ...
      // 2. 內測回饋
      if (txt === "內測回饋") {
        feedbackMode.set(uid, Date.now());
        await sendReply(event.replyToken,
          "感謝你參與 Ask Shumi 內測！\n\n請問對這個 Chatbot 有什麼建議？\n直接輸入你的想法即可，也歡迎上傳你想反饋的對話截圖 📸");
        logToSheet(uid, txt, "(開始內測回饋)");
        continue;
      }
      ...
      if (inFeedback) {
        feedbackMode.delete(uid);
        logToSheet(uid, `[內測建議] ${txt}`, "(內測回饋文字)");
        await sendReply(event.replyToken, FEEDBACK_THANKS);
        continue;
      }
