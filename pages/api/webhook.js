...
function logToSheet(userId, userMessage, botReply) {
  ...
  const timestamp = (() => { const d = new Date(Date.now() + 8 * 60 * 60 * 1000); return d.toISOString().slice(0, 19).replace("T", " "); })();
  fetch(LOGGER_URL, {
    method: "POST",
    redirect: "follow",
    headers: { "Content-Type": "text/plain" },
    body: JSON.stringify({ timestamp, userId, userMessage, botReply }),
...
// 處理滿意度回饋
if (userText === FEEDBACK_POSITIVE || userText === FEEDBACK_NEGATIVE) {
  const ackText = userText === FEEDBACK_POSITIVE
    ? "謝謝你的回饋，很高興有幫到你！有其他問題隨時可以問 🌿"
    : "謝謝告訴我們，我們會持續改善！還有問題歡迎繼續問，或寄信給 contact-hsc@mingyifoundation.org";
  await fetch("https://api.line.me/v2/bot/message/reply", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${TOKEN}` },
    body: JSON.stringify({ replyToken: event.replyToken, messages: [{ type: "text", text: ackText }] }),
  });
  logToSheet(userId, userText, "(回饋記錄)");
  continue;
}

// 一般問題：呼叫 Claude 並附上 Quick Reply
...
method: "POST",
headers: { "Content-Type": "application/json", Authorization: `Bearer ${TOKEN}` },
body: JSON.stringify({ replyToken: event.replyToken, messages: [{ type: "text", text, quickReply: QUICK_REPLY }] }),
});
const lineData = await lineRes.json();
console.log("LINE:", lineRes.status, JSON.stringify(lineData));
logToSheet(userId, userText, text);
