...
const FEEDBACK_COLLECTOR_PROMPT = `你是「回饋小幫手」，負責搜集 Ask Shumi 內測用戶的回饋。

你的任務：完整了解用戶的使用體驗，搜集清楚後，會統整給團隊進行評估和優化。

規則：
1. 用簡短、溫暖的台灣繁體中文，語氣像朋友
2. 若回饋不夠清楚，問一個具體澄清問題，一次只問一個
3. 不提出解決方案，只確認你理解用戶說的
4. 通常 1-2 輪澄清就夠，不要問太多
5. 若截圖是圖片，請詢問「這張截圖想反映什麼情況？」
6. 當回饋已完整時，感謝用戶，說明會統整給團隊，然後在回應最後加上 [FEEDBACK_COMPLETE]
7. 若用戶說沒了、謝謝等結束語，也視為完成並加 [FEEDBACK_COMPLETE]`;

const FEEDBACK_POSITIVE = "👍 有幫助";
const FEEDBACK_NEGATIVE = "👎 沒幫助";
const QR_SATISFACTION = {
  items: [
    { type:"action", action:{ type:"message", label:"👍 有幫助", text:"👍 有幫助" }},
    { type:"action", action:{ type:"message", label:"👎 沒幫助", text:"👎 沒幫助" }}
  ]
};

// process-level: userId -> { ts, sessionId, history }
const feedbackMode = new Map();
const FEEDBACK_TTL = 10 * 60 * 1000;
...
function logToSheet(userId, userMessage, botReply, sessionId) {
  if (!LOGGER_URL) return;
  fetch(LOGGER_URL, {
    method:"POST", redirect:"follow",
    headers:{"Content-Type":"text/plain"},
    body: JSON.stringify({ timestamp:ts(), userId, userMessage, botReply, sessionId })
  }).then(r=>console.log("LOGGER:",r.status)).catch(e=>console.log("LOGGER_ERR:",e.message));
}

async function logImageToSheet(userId, imageId, imageBase64, mimeType, botReply, sessionId) {
  if (!LOGGER_URL) return;
  fetch(LOGGER_URL, {
    method:"POST", redirect:"follow",
    headers:{"Content-Type":"text/plain"},
    body: JSON.stringify({
      timestamp:ts(), userId,
      userMessage: "[內測截圖 imageId:" + imageId + "]",
      botReply, sessionId, imageBase64, imageId, mimeType
    })
  }).then(r=>console.log("LOGGER_IMG:",r.status)).catch(e=>console.log("LOGGER_IMG_ERR:",e.message));
}

async function sendReply(replyToken, text, quickReply) {
...
export default async function handler(req, res) {
  ...
  for (const event of req.body.events || []) {
    ...
    const state = feedbackMode.get(uid);
    const inFeedback = state && (Date.now() - state.ts) < FEEDBACK_TTL;

    try {
      // 圖片（回饋模式）
      if (msgType === "image") {
        if (inFeedback) {
          // 下載截圖並存到 Drive（透過 Apps Script）
          ...
          } catch(imgErr) { console.log("IMG_DL_ERR:", imgErr.message); }

          state.history.push({ role:"user", content:"[用戶上傳了截圖]" });
          const r = await client.messages.create({
            model:"claude-haiku-4-5-20251001", max_tokens:300,
            system: FEEDBACK_COLLECTOR_PROMPT,
            messages: state.history
          });
          let reply = r.content[0].text;
          const done = reply.includes("[FEEDBACK_COMPLETE]");
          reply = reply.replace("[FEEDBACK_COMPLETE]","").trim();
          state.history.push({ role:"assistant", content:reply });
          if (done) feedbackMode.delete(uid);
          else feedbackMode.set(uid, { ...state, ts: Date.now() });
          await logImageToSheet(uid, event.message.id, imageBase64, mimeType, reply, state.sessionId);
      ...
      if (txt === "內測回饋") {
        ...
        feedbackMode.set(uid, { ts: Date.now(), sessionId, history: [] });
        const intro = "感謝你參與 Ask Shumi 內測！我是今天的回饋小幫手 😊\n\n我的任務是完整記錄你的使用體驗，搜集清楚後，會統整給團隊進行評估和優化。\n\n請直接說說你的感受，或上傳你想反映的截圖，我來幫你記錄 🌿";
        await sendReply(event.replyToken, intro);
        logToSheet(uid, txt, "(開始內測回饋)", sessionId);
        continue;
      }
      ...
      if (inFeedback) {
        ...
        logToSheet(uid, "[內測建議] " + txt, reply, state.sessionId);
        await sendReply(event.replyToken, reply);
        continue;
      }

      // 滿意度按鈕
      if (txt === FEEDBACK_POSITIVE || txt === FEEDBACK_NEGATIVE) {
      ...
      await sendReply(event.replyToken, text, QR_SATISFACTION);
      logToSheet(uid, txt, text);
